...
Tab:AddButton({
    Name = "重置界面位置",
    Callback = function()
        local centerX = workspace.CurrentCamera.ViewportSize.X / 2 - MainFrame.Size.X.Offset / 2
        local centerY = workspace.CurrentCamera.ViewportSize.Y / 2 - MainFrame.Size.Y.Offset / 2
...
Tab:AddToggle({
    ...
    Callback = function(enabled)
        if enabled then
            -- 禁用视觉效果
            for _, instance in ipairs(splib.Instances) do
...
Tab:AddToggle({
    ...
    Default = true,
    Callback = function(enabled)
        splib.Save.RememberSize = enabled
        SaveJson("sp library.json", splib.Save)
    end,
    __force_container = SettingTab
})
...
Tab:AddSlider({
    ...
    Callback = function(speed)
        ...
        getgenv().AnimationSpeedMultiplier = speed
    end,
    __force_container = SettingTab
})
Tab:AddToggle({
...
Tab:AddButton({
    ...
    Callback = function()
        local dialog = Window:Dialog({
            Title = "确认清除",
            Text = "这将清除所有保存的设置和数据，此操作不可撤销！",
            Options = {
                {"确认清除", function()
                
                    local files = {"sp library.json", "dropdowns.json", "binds.json", "sliders.json", "textboxes.json", "colorpickers.json"}
...
Tab:AddButton({
    Name = "导出配置",
    Callback = function()
        local configData = HttpService:JSONEncode(Flags)
